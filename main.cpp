 ...
 *
 * 複製:
 * 按下複製按鍵後,會跳出視窗,可以將圈選的圖形資料轉換成*.h檔給MCU使用
 *
 * 剪下：
 * 類似複製功能，但會清除被複製的區塊
 *
 * 貼上：
 * 按下後，會有預覽框會顯示，預設在左上角，預覽框可以拖動，按下Enter就可以貼在預覽框的位子
 *
 *　接下來就是undo 和redo了
 *　不過……
 *　在那之後呢？
...
int main(int argc, char *argv[])
{
    ...
    for (const QString &locale : uiLanguages) {
        const QString baseName = "SH1106_GUI_Design_sandbox_" + QLocale(locale).name();
